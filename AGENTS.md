...
- To make a library default *easy* to override, wrap it in `:where()` to zero out the class part. The panel group in the theme does this deliberately (`:where(.vgo-panel)`, `:where(.vgo-panel--flat)`, `:where(.vgo-panel--overlay, …)`). Panels are the primitive consumers keep reshaping, so their appearance is a default, not a rule. The cost is that panel modifiers now resolve by source order, so base must stay above modifiers. Button icon size is **not** a library default: icons follow the button font-size (1em inline SVG) and consumers opt into `--vgo-icon-*` sizes via `.vgo-u-icon-*` on the icon element.
